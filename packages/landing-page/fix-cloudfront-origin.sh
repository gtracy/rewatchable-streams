...
# Fix CloudFront distribution to point to rewatchables bucket with /webapp/ path
...
# Get the current config and update both origin and cache behavior
...
# Update the origin domain, path, and ID, plus the cache behavior target
jq '
  .Origins.Items[0].DomainName = "rewatchables.s3.amazonaws.com" |
  .Origins.Items[0].OriginPath = "/webapp" |
  .Origins.Items[0].Id = "S3-rewatchables" |
  .DefaultCacheBehavior.TargetOriginId = "S3-rewatchables"
